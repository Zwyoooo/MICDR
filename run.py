...
class Run():
    def __init__(self,
        ...
        self.ratio = [float(config['ratio'].split(',')[0]), float(config['ratio'].split(',')[1])]
    ...
    def get_optimizer(self, model):
        ...
        optimizer_meta = torch.optim.Adam(params=model.meta_net.parameters(), lr=self.lr, weight_decay=0.01)
    ...
    def CDR(self, model, data_src, data_map, data_meta, data_test,
        ...
        for i in range(self.epoch):
            ...
            self.update_results(mae, rmse, 'micdr')
            print('MAE: {} RMSE: {}'.format(mae, rmse))
